...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/blood-type'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/blood-type', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: BloodType) {
        return item.id;
    }
    registerChangeInBloodTypes() {
        this.eventSubscriber = this.eventManager.subscribe('bloodTypeListModification', (response) => this.loadAll());
    }
