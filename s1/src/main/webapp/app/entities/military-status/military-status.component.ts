...
@Component({
    selector: 'jhi-military-status',
    templateUrl: './military-status.component.html'
})
export class MilitaryStatusComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.militaryStatusService.search({
    ...
    transition() {
        this.router.navigate(['/military-status'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/military-status', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: MilitaryStatus) {
        return item.id;
    }
    registerChangeInMilitaryStatuses() {
        this.eventSubscriber = this.eventManager.subscribe('militaryStatusListModification', (response) => this.loadAll());
    }
