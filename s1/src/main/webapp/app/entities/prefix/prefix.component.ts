...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/prefix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: Prefix) {
        return item.id;
    }
    registerChangeInPrefixes() {
        this.eventSubscriber = this.eventManager.subscribe('prefixListModification', (response) => this.loadAll());
    }
